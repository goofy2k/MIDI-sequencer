...
//othe classes for hardware implementation 
 ...
 MQTTMidiIn :: MQTTMidiIn (unsigned int queueSizeLimit)
//MQTTMidiIn :: MQTTMidiIn (const std::string &clientName)
{      

   ESP_LOGW(TAG, "MQTTMidiIn constructor,queueSizeLimit: %u",queueSizeLimit );
        
  //taken from MidiInApi
  ...
  */
  inputData_->queue.ringSize = queueSizeLimit;  //error! was queue defined?
  if ( inputData_->queue.ringSize > 0 )
  inputData_->queue.ring = new MidiMessage[ inputData_->queue.ringSize ];
   
   //proceed with taking code from MidiInApi
   //<code here>
   //proceed with taking code lower on the heritage ladder, closer to hardware
     //<code here>
...
MQTTMidiIn :: ~MQTTMidiIn ()
{
    ...
    ESP_LOGW(TAG, "MQTTMidiIn destructor"); 
