...
*/
    try {//FCKX
        ...
        //port = new RtMidiOut();
        //ESP_LOGE(TAG,"executed creation of RtMidiOut port");
        ESP_LOGW(TAG,"start creation of MQTTMidiIn port");         
        port = new MQTTMidiIn(queue_size);
        ESP_LOGW(TAG,"new MQTTMidiIn object created");
        port->setCallback(HardwareMsgIn, this);
        ESP_LOGW(TAG,"MQTTMidiIn setCallback executed");
        port->ignoreTypes(false, true, true);
        ESP_LOGW(TAG,"MQTTMidiIn ignoreTypes set");
        ESP_LOGW(TAG,"executed creation of MQTTMidiIn port");
        }
