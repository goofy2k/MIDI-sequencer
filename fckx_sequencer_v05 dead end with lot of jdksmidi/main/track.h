...
//#include "midi.h"  //FCKX
//#include "msg.h"   //FCKX
#include "fckxMsg.h"   //FCKX
//#include "sysex.h" //FCKX
...
/// object is a fckxMIDITimedMessage. To avoid unnecessary copied of big events, access
...
public:
    ///
    /// GetEventAddress() const returns the address of the fckxMIDITimedMessage referred to by event_num
    ...
    ///
    const fckxMIDITimedMessage * GetEventAddress ( int event_num ) const;

    ///
    /// GetEventAddress()  returns the address of the fckxMIDITimedMessage referred to by event_num
    /// @param event_num an integer specifying an event number in the range 0 to MIDITrackChunkSize
    /// @returns The non-const pointer to the requested event.
    ///

    fckxMIDITimedMessage * GetEventAddress ( int event_num );

protected:

private:
    fckxMIDITimedMessage buf[MIDITrackChunkSize];
};
...
/// interface to the user that is useful for managing a list of fckxMIDITimedMessages. It internally
/// stores fckxMIDITimedMessage objects. There is a fixed maximum number of events that it can store,
...
public:
    ...
    ///
    /// The MIDITrack Destructor, frees all chunks and referenced fckxMIDITimedMessage's
    ///
    ~MIDITrack();
    ...
    ///
    /// Shrink() frees any unused MIDITrackChunk objects and associated fckxMIDITimedMessage events.
    ///
    void Shrink();
...
//  void  Sort();
    ...
    const fckxMIDITimedMessage *GetEvent ( int event_num ) const;
    fckxMIDITimedMessage *GetEvent ( int event_num );

    const fckxMIDITimedMessage *GetLastEvent() const
    {
        return GetEvent( GetNumEvents() - 1 );
    }
    unsigned long int GetLastEventTime() const //FCKX
    //MIDIClockTime GetLastEventTime() const
    {
       const fckxMIDITimedMessage *msg = GetLastEvent();
        return ( msg == 0 )? 0 : msg->GetTime();
        //return ( msg == 0 )? 0 : msg&.timestamp; //FCKX
    }

    bool GetEvent ( int event_num, fckxMIDITimedMessage *msg ) const;

    bool PutEvent ( const fckxMIDITimedMessage &msg );
/*  FCKX
...
*/
    ...
    bool PutEvent2 ( fckxMIDITimedMessage &msg );
    
    //FCKX
   // bool PutEvent ( const MIDITimedMessage &msg, const MIDISystemExclusive *sysex );
    bool SetEvent ( int event_num, const fckxMIDITimedMessage &msg );

    // put text message with known length (w/o ending NULL), or evaluate it if zero length
    bool PutTextEvent ( /*MIDIClockTime*/ unsigned long int time, int meta_event_type, const char *text, int length = 0 );

    bool MakeEventNoOp ( int event_num );

    bool FindEventNumber ( /*MIDIClockTime*/ unsigned long int time, int *event_num ) const;
...
// void  QSort( int left, int right );
    ...
    {
        int event_number;
        /*MIDIClockTime*/ unsigned long int time;
