...
//#include "jdksmidi/world.h"  //FCKX
//#include "jdksmidi/track.h"  //FCKX

#include "world.h"  //FCKX
...
const fckxMIDITimedMessage * MIDITrackChunk::GetEventAddress ( int event_num ) const
{
    return &buf[event_num];
}

fckxMIDITimedMessage * MIDITrackChunk::GetEventAddress ( int event_num )
{
    return &buf[event_num];
}
...
MIDITrack::MIDITrack ( const MIDITrack &t )
{
    ...
    {
        const fckxMIDITimedMessage *src;
...
bool MIDITrack::EventsOrderOK() const
{
    ...
    /*MIDIClockTime*/ long unsigned int time0 = GetEventAddress(0)->GetTime();

    for ( int i = 1; i < num_events; ++i )
    {
        /*MIDIClockTime*/ long unsigned int time1 = GetEventAddress(i)->GetTime();
        if ( time0 > time1 )
            return false;
        time0 = time1;
    }
...
int MIDITrack::RemoveIdenticalEvents( int max_distance_between_identical_events )
{
    ...
    {
        fckxMIDITimedMessage *mn = GetEvent( n );
        ...
        for (int i = 1; i < max_distance_between_identical_events; ++i)
        {
            ...
            fckxMIDITimedMessage *mni = GetEvent( n+i );
            if ( *mn == *mni )
            {
                ++removed;
                MakeEventNoOp( n );
...
const MIDITrack & MIDITrack::operator = ( const MIDITrack & src )
{
    if ( num_events == src.num_events )
    {
        for ( int n = 0; n < num_events; ++n )
        {
            const fckxMIDITimedMessage * msg = src.GetEventAddress( n );
            SetEvent( n, *msg );
        }
    }
    else
    {
        ...
        {
            const fckxMIDITimedMessage *msg = src.GetEventAddress ( i );
...
{
    Clear();
    const fckxMIDITimedMessage *ev1;
    int cur_trk1ev = 0;
    int num_trk1ev = src1->GetNumEvents();
    const fckxMIDITimedMessage *ev2;
    int cur_trk2ev = 0;
    int num_trk2ev = src2->GetNumEvents();
    /*MIDIClockTime*/ long unsigned int last_data_end_time = 0;
...
fckxMIDITimedMessage * MIDITrack::GetEventAddress ( int event_num )
{
    return chunk[ event_num/ ( MIDITrackChunkSize ) ]->GetEventAddress (
               ( event_num % MIDITrackChunkSize ) );
}

const fckxMIDITimedMessage * MIDITrack::GetEventAddress ( int event_num ) const
{
    return chunk[ event_num/ ( MIDITrackChunkSize ) ]->GetEventAddress (
               ( event_num % MIDITrackChunkSize ) );
}

bool MIDITrack::PutEvent ( const fckxMIDITimedMessage &msg )
{
...
bool MIDITrack::PutEvent ( const MIDIDeltaTimedBigMessage &msg )
{
    ...
    fckxMIDITimedMessage m = msg2;
     long unsigned int t = msg.GetDeltaTime() + GetLastEventTime();
    m.SetTime( t );
    return PutEvent( m );
}
*/

bool MIDITrack::PutEvent2 ( fckxMIDITimedMessage &msg )
{
    if ( PutEvent ( msg ) )
    {
        /*MIDIClockTime*/ long unsigned int t = msg.GetTime();
        msg.Clear();
        msg.SetTime( t );
        return true;
    }
...
{
    fckxMIDITimedMessage m ( msg, sysex );
    return PutEvent ( m );
}
*/
/*
bool MIDITrack::PutTextEvent (  long unsigned int time, int meta_event_type, const char *text, int length )
{
...
bool MIDITrack::GetEvent ( int event_num, fckxMIDITimedMessage *msg ) const
{
...
bool MIDITrack::SetEvent ( int event_num, const fckxMIDITimedMessage &msg )
{
    if ( !IsValidEventNum( event_num ) )
    {
        return false;
...
bool MIDITrack::MakeEventNoOp ( int event_num )
{
    ...
    {
        fckxMIDITimedMessage *ev = GetEventAddress ( event_num );
        if ( ev )
            ev->SetNoOp();
        return true;
    }
...
bool MIDITrack::FindEventNumber ( /*MIDIClockTime*/ long unsigned int time, int *event_num ) const
{
    ...
    {
        const fckxMIDITimedMessage *msg = GetEventAddress ( i );
...
const fckxMIDITimedMessage *MIDITrack::GetEvent ( int event_num ) const
{
    if ( !IsValidEventNum( event_num ) )
    {
        return 0;
...
fckxMIDITimedMessage *MIDITrack::GetEvent ( int event_num )
{
    if ( !IsValidEventNum( event_num ) )
    {
        return 0;
